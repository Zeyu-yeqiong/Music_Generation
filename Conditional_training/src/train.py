...
from musegan.data import load_data, get_dataset, get_samples,load_label
...
def load_training_data(params, config):
    ...
    if params['is_conditional']:
        labels=load_label(config['label_filename'])
    else:
        labels = None
...
def load_or_create_samples(params, config):
    ...
    if params['is_conditional']:
        if os.path.exists(sample_y_path):
            ...
            if sample_y.shape[1] != params['latent_dim']:
                LOGGER.info("Loaded sample_y has wrong shape")
                yresample = True
            else:
                yresample = False
        else:
    ...
    if yresample:
        ...
        sample_y = scipy.stats.truncnorm.rvs(
            -2, 2, size=(np.prod(config['sample_grid']), params['ylatent_dim']))
        make_sure_path_exists(config['model_dir'])
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        while step < config['steps']:
            ...
            if ((config['save_samples_steps'] > 0)
                ...
                feed_dict_sampler = {placeholder_z: sample_z,placeholder_y: sample_y}
            ...
            if ((config['evaluate_steps'] > 0)
                ...
                feed_dict_evaluation = {
                    ...
                    placeholder_y:scipy.stats.truncnorm.rvs(-2, 2, size=(
                        np.prod(config['sample_grid']), params['ylatent_dim']))}
                if params['is_accompaniment']:
