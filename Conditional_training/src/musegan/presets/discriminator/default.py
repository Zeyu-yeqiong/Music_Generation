    ...
    def __call__(self, tensor_in, condition=None, training=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('shared'):
                h = conv_layer(h, 128, (1, 4, 3), (1, 4, 2))         #bs* 4, 4, 3
                h = conv_layer(h, 256, (1, 4, 3), (1, 4, 3))         #bs* 4, 1, 1

            # Chroma stream
            with tf.variable_scope('chroma'):
                c = conv_layer(chroma, 32, (1, 1, 12), (1, 1, 12))   #bs* 4, 4, 1
                c = conv_layer(c, 64, (1, 4, 1), (1, 4, 1))          #bs* 4, 1, 1
            ...
            with tf.variable_scope('on_off_set'):
                ...
                o = conv_layer(o, 64, (1, 4, 1), (1, 4, 1))          # 4, 1, 1
                                                       #?, 8, 1, 1, 256
            h = tf.concat((h, c, o), -1)                             #8, 1, 1, 64
            
            # Merge all streams
            with tf.variable_scope('merged'):
                h = conv_layer(h, 512, (2, 1, 1), (1, 1, 1))         # bs,7, 1, 1,512
            
 
            h = tf.reshape(h, (-1, 3584))       #bs*3584

            print('-----------dis------------')
            print(h.shape)

            print(condition.shape)
            h = dense(h,256)       #bs*256
            h=  tf.concat([h,condition],1)      #bs*(256+condition)
            h=dense(h,8)
