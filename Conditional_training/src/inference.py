...
def main():
    ...
    # ================================== Data ==================================
    if params['is_accompaniment']:
        data = load_data(config['data_source'], config['data_npz_name'])
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        # Run sampler op
        LOGGER.info("config['runs']:  ",config['runs'])
        for i in range(config['runs']):
            ...
            npz_name.append('./exp/my_experiment/results/inference/pianorolls/fake_x_bernoulli_sampling/fake_x_bernoulli_sampling_'+str(i)+'.npz')
            npz_name2.append('./exp/my_experiment/results/inference/pianorolls/fake_x_hard_thresholding/fake_x_hard_thresholding_'+str(i)+'.npz')
